...
const FileExplorer: React.FC<FileExplorerProps> = ({
  ...
  // Track if we're at the end of the scroll
  const [isAtEnd, setIsAtEnd] = useState(false);

  // Track if we have more items to potentially load
  const [hasMoreItems, setHasMoreItems] = useState(true);

  // State to track virtual window and visible items
  const [visibleItems, setVisibleItems] = useState<(FolderItem | FileItem)[]>([]);
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (contentRef.current) {
        ...
        // Check if we're near the end of the scroll (within 200px)
        const isNearEnd = scrollTop + clientHeight >= scrollHeight - 200;
        setIsAtEnd(isNearEnd);

        // If we're near the end and have more items, this would be where you'd load more
  ...
  // Update visible items when currentItems changes
  useEffect(() => {
    if (currentItems.length > 0) {
      setVisibleItems(currentItems);
      setHasMoreItems(false); // Set this based on your pagination logic
    } else {
      setVisibleItems([]);
    }
  }, [currentItems]);
                    ...
                    <div className="ml-3 truncate flex-1">
                      {renamingItemId === item.id ? (
                          ...
                          <p className="font-medium text-gray-800 truncate">
                            {item.type === 'file' ? (item as FileItem).original_name : item.name}
                          </p>
                      )}
                      {item.type === 'file' && (
  ...
  return (
      <div className="h-full flex flex-col bg-gray-50">
            ...
            {breadcrumbs.map((crumb, index) => (
                <div key={`breadcrumb-${index}`} className="flex items-center flex-shrink-0">
                  <button
                      ...
                      className="hover:text-blue-600 text-sm font-medium transition-colors breadcrumb-item px-2 whitespace-nowrap"
                  >
                    {crumb.name}
                  </button>
        ...
        {/* "New Folder" controls - Also fixed */}
        <div className="px-6 pt-4 pb-3 flex-shrink-0 bg-gray-50 border-b border-gray-100">
          {isCreatingFolder ? (
        ...
        {/* Main content area with folders and files - Scrollable container */}
        <div
            ref={contentRef}
            className="flex-grow overflow-y-auto h-0 px-6 py-4"
            style={{
        ...
        >
          ...
          {/* ScrollToTop button that appears when scrolling down */}
          {scrollPosition > 200 && (
              <button
                  className="fixed bottom-6 right-6 bg-gray-800 bg-opacity-60 hover:bg-opacity-80 text-white rounded-full p-3 shadow-lg transition-all"
                  onClick={() => {
                    if (contentRef.current) {
